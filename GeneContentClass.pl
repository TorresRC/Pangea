...
my($Project, $BoleanFileName, $MetaDataFileName, $ProbabilitiesFile, $nBoleanFile, $Line,
   ...
   $StrainHits, $ProbeHit, $ProbeHits, $nHits, $ClassCoreGenes);
my($i, $j);
my(@BoleanFile, @BoleanFileFields, @BoleanTable, @MetaDataField, @MetaDataFile,
   @MetaDataFileFields, @MetaData, @Classes, @Strains);
my(%StrainClass, %Classes, %pClasses, %cpClasses, %ClassHits, %ProbeClass, %CoreGenome, %PanGenome);
my $BoleanTable = [ ];
my $Probabilities = [ ];
...
#Total of hits on the entire bolean teable
...
for ($i=1;$i<$nBoleanFile;$i++){
	...
	for ($j=0; $j<scalar@Classes;$j++){
		$Class = $Classes[$j];
                if ($ProbeClass{$Probe}{$Class} eq $Classes{$Class}){
                        $ClassCoreGenes = $Project ."/". $ProjectName ."_". $Class ."_CoreGenes.txt"; 
                        open (FILE, ">>$ClassCoreGenes");
                                print FILE $Probe, "\n";
                        close FILE;
                        $CoreGenome{$Class}++;
                }else{
...
open (PROBABILITIES, ">$ProbabilitiesFile");
open (CLASSGENECONTENT, ">$ProbabilitiesFile");
for ($i=0; $i<$nBoleanFile; $i++){
